...
struct PointDescriptor {
    ...
    PointDescriptor &operator[](int idx) {
        return *this;
    }
...
template <typename T>
void mergeSort(DynamicArray<T> &array, int n) {
    int curr_size;
    int left_start;
...
void solve(DynamicArray<PointDescriptor> &point_descriptor_array, std::ostream &output_stream) {
    ...
    int total_filled = 0, section_id = 0;
    mergeSort(point_descriptor_array, point_descriptor_array.getSize());

    for (int i = 0; i < point_descriptor_array.getSize(); ++i) {
        int old_section_id = section_id;
        if (point_descriptor_array[i].isSectionStarted) {
            ++section_id;
        } else {
            if (old_section_id != 0 && section_id) 
                ++total_filled;
            if (section_id - 1 >= 0) {
                --section_id;
            }
